 ...
 * Created once at startup and destroyed at shutdown.
 */
public final class VulkanContext {
    ...
    public static VulkanContext create(long windowHandle) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (VALIDATION) {
                ...
                debugMessenger = createDebugMessenger(instance, callbackHandle, stack);
            }
            ...
            int graphicsFamily = findQueueFamily(physicalDevice, VK_QUEUE_GRAPHICS_BIT, -1, surface, false, stack);
            int presentFamily = findQueueFamily(physicalDevice, 0, graphicsFamily, surface, true, stack);
            VkDevice device = createDevice(physicalDevice, graphicsFamily, presentFamily, stack);
            VkQueue graphicsQueue = getQueue(device, graphicsFamily, stack);
            VkQueue presentQueue = getQueue(device, presentFamily, stack);
    ...
    private static long createDebugMessenger(VkInstance instance, VkDebugUtilsMessengerCallbackEXT cb, MemoryStack stack) {
    ...
    private static VkPhysicalDevice pickPhysicalDevice(VkInstance instance, long surface, MemoryStack stack) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (isDeviceSuitable(candidate, surface, stack)) {
    ...
    private static boolean isDeviceSuitable(VkPhysicalDevice device, long surface, MemoryStack stack) {
        try {
            findQueueFamily(device, VK_QUEUE_GRAPHICS_BIT, -1, surface, false, stack);
            findQueueFamily(device, 0, -1, surface, true, stack);
    ...
    private static int findQueueFamily(
            ...
            long surface, boolean requirePresent, MemoryStack stack
    ) {
        IntBuffer countBuf = stack.mallocInt(1);
    ...
    private static VkDevice createDevice(VkPhysicalDevice physicalDevice, int graphicsFamily, int presentFamily, MemoryStack stack) {
    ...
    private static VkQueue getQueue(VkDevice device, int family, MemoryStack stack) {
